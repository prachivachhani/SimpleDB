...
public class TupleDesc {
	...
	{
		// For an index to be valid one, it has to be between 0 and the total length of the types.		
		if (i >= 0 && i < types.length)
			// returns the type of the individual ith field.
			return types[i];
		// exception to be thrown if index is less than 0 or more than the number of fields.	
        throw new NoSuchElementException("invalid field index");
	}
	...
	public String toString()
	{
		...
		//loop through the tuple description array and add the types and names in the given format.		
		for(int i=0; i<types.length; i++)
		{
			//if its the last fieldType(fieldName) pair then it should not have pipe(|) symbol. 
			if(i == types.length-1)
