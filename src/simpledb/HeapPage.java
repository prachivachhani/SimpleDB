...
public class HeapPage implements Page, Iterable<Tuple> {
	...
	public Tuple getTuple(int entryID) 
	{
		...
		//Check the validity of the entryID within the limits of the byte array.
		//Check whether that tuple is already deleted or not (i.e. if location==-1).
		if(entryID<0 || entryID >= entryCount() || location==-1) 
		{
			return null;
		}
		//Get the data of the tuple in a byte array, create a new tuple from that data and then set its recordID giving pid and its entryID as parameters. 
	...
	/**
	 * Deletes the specified {@code Tuple} from this {@code HeapPage}; 
	 * the {@code Tuple} should be updated to reflect
	 * that it is no longer stored on any page.
	 * 
	 * @throws DbException
	 *             if the {@code Tuple} is not on this {@code HeapPage}, 
	 *             or the {@code Tuple} slot is already empty.
	 * @param t
	 *            the {@code Tuple} to delete
	 */
	// code referenced from black board jar file.
	...
	/**
	 * Adds the specified {@code Tuple} to this {@code HeapPage}; 
	 * the {@code Tuple} should be updated to reflect that it
	...
	/**
	 * Generates a byte array corresponding to an empty {@code HeapPage}. 
	 * This method is used to add new, empty pages to the file. 
	 * Passing the results of this method to the {@code HeapPage} constructor will create a {@code HeapPage}
	...
	/**
	 * Finds (by examining the header of this {@code HeapPage}) the location
	 *  of the tuple at the specified entry.
	 * 
	 * @param entryID
	 *            the ID of the entry.
