...
public class BufferPool {
	...
	{
		//get the value of the page attached with the pid with the hash map variable.
		Page page = id2page.get(pid);
		
		//Check if the buffer pool is full. If it is full, throw DbException.
		if(id2page.size() > numPages)
		{
			throw new DbException("Page requests exceeds the pages in the buffer.");
		}
		
		//Return the page if it is present in buffer pool.
		if(page!=null)
		{
			return page;
		}
		
		//Else get the required page from DbFile and then read from it.
		else 
		{
